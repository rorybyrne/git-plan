...
class UIService:
    ...
    def choose_plan(plans: List[Plan], message: str):
        ...
        options = [
            inquirer.List("plan", message=message, choices=[(c.message.headline, c) for c in plans])
        ]
    ...
    def render_plans(self, plans: List[Plan], headline_only=True):
        ...
        max_headline_length = max(len(p.message.headline) for p in plans)
        for plan in plans:
            self._render_plan(plan, str(plan.id), headline_only, max_headline_length)
        self.print(f"\n\n{len(plans)} plans total.")

    def _render_plan(self, plan: Plan, tag: str, headline_only: bool, max_healine_length: int):
        if not plan.updated_at:
            raise ValueError("Invalid plan: no updated_at field found.")

        time_display = (
            f"[dim]({humanize.naturaltime(dt.datetime.fromtimestamp(plan.updated_at))})[/dim]"
        )
        if headline_only:
            self.print(
                f"[bold][{tag}][/bold] {plan.message.headline:{max_healine_length + 10}s} {time_display}"
            )
        else:
