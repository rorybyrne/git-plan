...
PLAN_NO_TIME = {"branch": "foo", "message": {"headline": "headline", "body": "body"}, "id": "TST-1"}
...
def test_plan_should_auto_populate_time_fields_on_load_if_missing():
    with tempfile.TemporaryDirectory() as tempdir:
        file_name = Path(tempdir, "commit-12345.txt")
        with open(file_name, encoding="utf8", mode="a") as f:
...
def test_plan_should_not_auto_populate_existing_time_fields():
    with tempfile.TemporaryDirectory() as tempdir:
        file_name = Path(tempdir, "commit-12345.txt")
        with open(file_name, encoding="utf8", mode="a") as f:
